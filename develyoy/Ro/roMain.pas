...
begin
 if first then
  begin
   ...
   if FileExists(fn+'udl') then
    udl:='File name='+fn+'udl'
   else
    if FileExists(fn+'mdb') then
     udl:='Provider=Microsoft.Jet.OLEDB.4.0;Data Source='+fn+'mdb'
    else
     begin
      //No DB found!
...
begin
 ...
 while not(rs.EOF) do
  begin
   ...
   m.Caption:=VarToStr(rs.Fields.Item['network_name'].Value);
   m.Tag:=rs.Fields.Item['network_id'].Value;
   m.OnClick:=QConClick;
   rs.MoveNext;
  end;
 rs:=nil;
