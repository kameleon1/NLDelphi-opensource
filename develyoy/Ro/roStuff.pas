...
type
 ...
 TRoParsedLine=class
  ...
  public
   ...
   constructor Create(DoText:string);
 ...
 public
  constructor Create(Msg:string;Code:string='');
 end;
 TRoNetworkLine=class(TRoParsedLine)
 public
  constructor Create(Socket: TTCPClient;Msg:string);
 end;
     ...
     end;
{
 ...
 re.Pattern:='((http|mailto|ftp|news):.|www\.[-a-z0-9]+\.[a-z][a-z][a-z]?)([^\s"'')\]&<>]|&amp;)*';
...
constructor TRoParsedLine.Create(DoText:string);
begin
 inherited Create;
 MessageType:=cmIrc;
 origin:='';
...
procedure TRoParsedLine.Parse(s:string);
var
 i,j:integer;
...
constructor TRoErrorLine.Create(Msg:string;Code:string='');
begin
 inherited Create(Msg);
 MessageType:=cmError;
 if not(Code='') then code:=Code;
 PrepHTML;
end;

constructor TRoNetworkLine.Create(Socket:TTCPClient;Msg:string);
begin
  inherited Create(Msg);
 MessageType:=cmNetwork;
 origin:=Socket.LocalHostName+' '+Socket.LocalHostAddr+':'+Socket.LocalPort;
end;
