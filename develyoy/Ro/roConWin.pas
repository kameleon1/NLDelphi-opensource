...
procedure TConnectionWin.csError(Sender: TObject; Socket: TCustomWinSocket;
  ErrorEvent: TErrorEvent; var ErrorCode: Integer);
var
 pl:TRoParsedLine;
begin
  inherited;
 pl:=TRoParsedLine.Create('Error: '+SysErrorMessage(ErrorCode));
...
begin
 ...
 FullName:=VarToStr(rs.Fields.Item['network_fullname'].Value);
 EMail:=VarToStr(rs.Fields.Item['network_email'].Value);
 i:=1;
...
begin
  inherited;
 if not(cs.Active) then DoConnect;
end;
...
begin
 s:=Nick;
 if not(ModeFlags='') then s:=s+' ['+ModeFlags+']';
 if not(NetworkName='') then s:=s+' ('+NetworkName+')';
 s:=s+' '+ServerName;
 SetCaption(s);
end;
...
begin
 inherited;
 WasHigh:=false;
 if cs.Active then SetIcon(iiConOpen) else SetIcon(iiConClosed);
end;
