...
begin
  ...
  begin
    ...
    if not (csDesigning in ComponentState) or
      ((csDesigning in ComponentState) and (Parent <> nil)) then RecreateWnd;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      LParent := nil;
      if (fsModal in FormState) and (FPopupMode = pmNone) then
        LPopupMode := pmAuto
      ...
      if (FInternalPopupParent = nil) and (FInternalPopupParentWnd = 0) then
        case LPopupMode of
//          pmNone: WndParent := Application.Handle;
          pmNone, pmAuto:
            begin
      ...
      case CreateStyle of
        ...
        bsSizeable, bsSizeToolWin:
          begin
            ...
            if (not(csDesigning in ComponentState) or (Parent = nil)) and (FPosition in [poDefault, poDefaultSizeOnly]) then
            begin
...
function DestroyPopupWindow(Window: HWND; FOwner: HWND): BOOL; stdcall;
begin
  if (Window <> FOwner) and (FindControl(Window) = nil) and
    (GetWindow(Window, GW_OWNER) = FOwner) then
    SendMessage(Window, CM_DESTROYHANDLE, 0, 0);
  Result := True;
end;
...
begin
  ...
  begin
    ...
    EnumThreadWindows(GetCurrentThreadID, @DestroyPopupWindow, WindowHandle);
  end;
...
begin
  ...
  if (FDesigner <> nil) and (csDesigning in ComponentState) and
    (Parent <> nil) and (Menu <> nil) and (Menu.Items.Count > 0) then
  begin
