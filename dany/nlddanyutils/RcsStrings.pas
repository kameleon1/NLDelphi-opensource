...
14-08-2022:  * Changed "ToString" into "To_String" to prevent compiler warning.
09-09-2022:  * Added Pos_ic and PosN_ic (Pos and PosN with case ignored)
}
...
type
  ...
  public
    ...
    function To_String(const Separator: Char = ','): string;
...
function Pos_ic(const SubStr, Source: string): integer;
// Same as pos but ignore case

function PosN_ic(const SubStr, Source: string; const P: Integer = 1): integer;
// Same as PosN but ignore case
...
function TRcsStrings.To_String(const Separator: Char = ','): string;
begin
...
function Pos_ic(const SubStr, Source: string): integer;
begin
  Result := pos(uppercase(SubStr), Uppercase(Source));
end;

// Same as PosN but ignore case
function PosN_ic(const SubStr, Source: string; const P: Integer = 1): integer;
begin
  Result := PosN(Uppercase(SubStr), uppercase(Source), P);
end;
...
begin
  OldShortDateFormat := System.SysUtils.FormatSettings.ShortDateFormat;
  System.SysUtils.FormatSettings.ShortDateFormat := 'yymmdd';
  Result := DateToStr(Date);
  System.SysUtils.FormatSettings.ShortDateFormat := OldShortDateFormat;
end;
...
begin
  OldShortDateFormat := System.SysUtils.FormatSettings.ShortDateFormat;
  System.SysUtils.FormatSettings.ShortDateFormat := 'yyyymmdd';
  Result := DateToStr(Date);
  System.SysUtils.FormatSettings.ShortDateFormat := OldShortDateFormat;
end;
...
begin
  OldShortDateFormat := System.SysUtils.FormatSettings.ShortDateFormat;
  System.SysUtils.FormatSettings.ShortDateFormat := 'dd-mm-yy';
  Result := DateToStr(Date);
  System.SysUtils.FormatSettings.ShortDateFormat := OldShortDateFormat;
end;
...
begin
  OldLongTimeFormat := System.SysUtils.FormatSettings.LongTimeFormat;
  System.SysUtils.FormatSettings.LongTimeFormat := 'hh:nn:ss';
  Result := TimeToStr(Time);
  System.SysUtils.FormatSettings.LongTimeFormat := OldLongTimeFormat;
end;
...
begin
  ...
  Dp := Pos(System.SysUtils.FormatSettings.DecimalSeparator, Result);
  ...
  begin
    ...
    Insert(System.SysUtils.FormatSettings.DecimalSeparator, Result, 2);          // insert new decimalseparator
  end else                                        // Exponent > 0
  begin
...
var ValidChars, ValidLeadingChars: set of Char;
  I: integer;
begin
  ValidChars := ['0'..'9'] + [System.SysUtils.FormatSettings.DecimalSeparator];
...
begin
  ...
  Tmp := StringReplace(Tmp, '/', System.SysUtils.FormatSettings.DateSeparator, [rfReplaceAll]);
  Tmp := StringReplace(Tmp, '-', System.SysUtils.FormatSettings.DateSeparator, [rfReplaceAll]);
